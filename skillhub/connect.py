"""Native cognis-connect emit for skillhub — forward findings to any platform.

Maps skillhub's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    skillhub ... --format json | skillhub-emit --to stix
    skillhub-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        ...
        out['severity'] = rec.get('priority', 'info')
        out['type'] = rec.get('category', '')
        out['description'] = rec.get('summary', '')
