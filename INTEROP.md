...
How **skillhub** fits the wider Cognis suite — a composable set of defensive, analytical
tools that all run on your own hardware and speak plain **JSON** (or an OpenAI-compatible
**`/v1`**). `skillhub` lives in the **Agents + cognition** cluster.
...
| from | relation | to |
|---|---|---|
| `skillhub` | JSON findings compose with | [`agentlex`](https://github.com/cognis-digital/agentlex), [`humind`](https://github.com/cognis-digital/humind), [`agentsmith`](https://github.com/cognis-digital/agentsmith), [`agentmap`](https://github.com/cognis-digital/agentmap), [`engram`](https://github.com/cognis-digital/engram) |
| `skillhub` | AI add-ins are served `/v1` by | [`edgemesh`](https://github.com/cognis-digital/edgemesh) (your fleet) |
| `skillhub` | findings can be narrated / reasoned over by | [`humind`](https://github.com/cognis-digital/humind) -> [`agentlex`](https://github.com/cognis-digital/agentlex) |
| `skillhub` | export to intel formats via | [`stixgen`](https://github.com/cognis-digital/stixgen) (STIX) / [`attackmap`](https://github.com/cognis-digital/attackmap) (ATT&CK) |

## Composition patterns

Everything reads/writes JSON, so tools chain with ordinary pipes; nothing leaves the box.

**1 — chain within the agents + cognition cluster.** `skillhub` output feeds the next tool:
```bash
skillhub ... --format json > out.json          # this tool's findings (see `skillhub --help`)
agentlex ... < out.json                      # the cluster sibling consumes them
```
...
skillhub ...                                         # vision / reasoning add-ins light up
```

**3 — export findings to your SOC's formats.**
```bash
skillhub ... --format json | stixgen from-json > bundle.stix.json   # STIX 2.1
skillhub ... --format json | attackmap map > attack.json            # ATT&CK techniques
```

**4 — narrate through cognition + agents.** `humind` extracts salience from `skillhub`'s
output; `agentlex` holds it as KB facts and fires Horn rules to escalate. `skillhub` slots
into the **Cognition + agents**, **Private-AI backbone** stacks below.
